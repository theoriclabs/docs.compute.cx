...
`--gpu H100` is an alias for reserved `H100-SXM`. `--gpu MI300X` is the AMD SKU. `--provider vastai --gpu RTX-4090` is a cheaper interruptible card. `--yes` skips the interactive spend confirmation.
...
| `--provider` | Optional pin (`runpod`, `hotaisle`, `vastai`). Required when a SKU name exists on more than one provider. |
