...
Save [`simple_mlp.py`](/examples/simple_mlp.py) first, or copy it from [First run](/get-started/first-run). `--yes` skips the interactive spend confirmation; the run still burns prepaid credit. Use `--dry-run` in place of `--wait --yes` to print the upload plan without creating a machine.
