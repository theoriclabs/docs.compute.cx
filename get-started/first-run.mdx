...
A new run can be refused by the account's one-active-run, spend, or create-rate limits, or by provider availability, even with a positive balance. Keep the request id from the error body and file a durable report with `compute report "Run create was refused" --details "request id: …"`. [Support](/support) explains what to include and how to follow the reply.
